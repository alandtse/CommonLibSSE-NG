...
#define VR_PLAYER_RUNTIME_DATA_CONTENT                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               \
	...
	std::uint64_t      unk6A8[0x5];                                                                   /* 6A8 - 40 bytes; VR PC ctor inits +0x08 = π (0x40490FDB), +0x0C = byte 1; likely angle/limit struct. Last element's low byte is read as a bool gate in PlayerCharacter::UpdateRoomscaleMovement alongside playerFlags.flags6&0x20 (binary-verified in Ghidra) */                                                                                                                                                                                                                                                                                            \
	std::uint32_t      unk6D0;                                                                        /* 6D0 */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
	std::uint32_t      isRightHandMainHand;                                                           /* 6D4 - Likely derived from bLeftHandedMode:VRInput (confirmed real ini setting via binary string table; no "MainHand" setting exists -- prior comment's claim was wrong). Exact write site not yet traced */                                                                                                                                                                                                                                                                                                                                                 \
	std::uint32_t      isLeftHandMainHand;                                                            /* 6D8 - see isRightHandMainHand */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            \
	std::uint32_t      unk6DC;                                                                        /* 6DC */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
	std::uint64_t      unk6F0[0x3];                                                                   /* 6F0 - 24 bytes (corrected from a stale [0x4]/32-byte declaration -- the 4th element didn't exist; it was shifting hands/unk8B0/questTargetsLock 8 bytes late). Confirmed via VR PC ctor decompile: exactly 6 float writes at +0x0/+0x4/+0x8/+0xC/+0x10/+0x14, each zeroed via the same NiPoint3-zero-component globals used elsewhere in the ctor -- 2 NiPoint3, no 4th element */                                                                                                                                                                          \
	VRPlayerHandData   hands[2];                                                                      /* 710 - per-controller state history (binary-verified via vector_constructor_iterator in PC ctor) */                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
	std::uint64_t      unk8B0[0x22];                                                                  /* 8B0 - 272 bytes; misc VR state. Ctor writes +0x00..+0x27 directly (0xFFFFFFFF sentinel, -FLT_MAX sentinel, two 25.0 floats, zeros) then calls VRPlayerViewState_Init with a pointer to +0x28, NOT +0x00 (live+disassembly-verified: LEA RCX,[this+0x8D8], i.e. this field's own start +0x28) — so the FOV/matrix/point layout below is relative to +0x28: +0x10(=abs +0x38) float 75.0 (likely FOV), +0x14(=abs +0x3C) NiMatrix3 Identity, +0x38(=abs +0x60) NiMatrix3 Identity, +0x5C..+0x8B(=abs +0x84..+0xB3) 4× NiPoint3 — VR stereo view state */ \
	...
	PlayerFlags            playerFlags;                                                               /* 12D0 */
		...
		// VR's block starts 0x18 later than SE's 0x3D8 anchor -- VR has 3 extra BSTEventSink members
		// (vrOverlayChangeEventSink/vrDeviceConnectionChangeEventSink/vrResetHMDHeightEventSink) SE
		// doesn't. Matches GetVRNodeData's own (correct) 0x3F0 anchor below; binary-verified in Ghidra.
