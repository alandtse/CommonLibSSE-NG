...
#define VR_PLAYER_RUNTIME_DATA_CONTENT                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
	...
	std::uint64_t      unk8B0[0x22];                                                                  /* 8B0 - 272 bytes; misc VR state. +0x00-0x27 ctor sentinels/floats. +0x28-0x37 (4 floats): fPlayer1stPersonLagNode_Max{Left,Right,Up,Down}Angle_VRUI. +0x38 float: fPlayer1stPersonLagNode_VerticalThreshold_VRUI. Applied by ApplyVR1stPersonLagNodeAngleSettings. +0x3C on (VRPlayerViewState_Init): +0x3C/+0x60 NiMatrix3 Identity, +0x84..+0xB3 4x NiPoint3 -- VR stereo view state. Byte-granular sub-offsets beyond +0xB3 (index N here means byte offset N, not element N): +0x14 cached character Z rotation; +0x18 gamepad-heading-smoothing gate; +0xC0 gamepad-angle-smoothing gate; +0xC8 raw input angle; +0xD4 wrapped output angle (MapMenu follow-node orientation); +0xE0 NiPoint3, zero-initialized in the ctor; +0xEC NiPoint3 read by UpdateVRComfortSwim (bComfortSwim_VR gate); +0x108 byte pending-flag set by PlayerCharacter::ResetVRPresentationNodes; +0x109 upright-yaw gate; +0x10D sitting-height gate (UpdateVRHeadTracking/UpdateVRMapMenuFollowNode). Not yet split into named sub-fields. */ \
	...
	std::uint64_t          unk1240[0x11];         /* 1240 - 136 bytes; PERK ENTRY POINT CACHE. PC+0x1288 (alchemy_ptr) read by EPAlchemyEffectHasKeyword; PC+0x1290 (tempering_ptr) read by EPTemperingItemHasKeyword/EPTemperingItemIsEnchanted. First entry (0x1248-0x1297, 0x50 bytes) is a full struct; the second (0x1298 on) uses a different, shorter init pattern that may extend into preTransformationData -- not a clean array of 2 identical structs, needs more work before typing */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
